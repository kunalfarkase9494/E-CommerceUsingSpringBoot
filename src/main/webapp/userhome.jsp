...
<div class="main-navbar shadow-sm sticky-top">
        <div class="top-navbar bg-dark">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-2 my-auto d-none d-sm-block d-md-block d-lg-block">
                    <div class="d-flex">
                    ...
                    <div class="col-md-5 my-auto">
                        <form role="search" action="search" method="post">
                            <div class="input-group">
                                <input type="search" name="category" placeholder="Search your product" class="form-control" />
...
<%
List<Products> list = (List<Products>)request.getAttribute("plist");
if(list!=null){
	int count=0;
...
<div class="row">
			<%if(i<list.size()){ %>
              <div class="col-md-4 bg-light">
                <div class="card">
                  <img class="card-img-top" src="images/<%=list.get(i).getPfile()%>" alt="Product 1">
                  <div class="card-body">
                    <form action="pinfo" method="post">
                  	<h5 class="card-title">Product Name :- <%=list.get(i).getPname()%></h5>
                    <h6 class="card-text">Price :- Rs.<%=list.get(i).getPrice()%></h6>
                    <p class="text-secondary">Discount :-<%=list.get(i).getPdis() %>%</p>
                    	<input type="hidden" name="pname" value="<%=list.get(i).getPname()%>">
                   		<input type="hidden" name="uname" value="<%=check%>">
                   		<input type="hidden" name="quantity" value="<%=1%>">
                   		<input type="hidden" name="adname" value="<%=list.get(i).getUname()%>">
						<button class="btn btn-sm btn-outline-secondary mb-2" >More Info..>></button>		
					</form> 
                    <%
					if(check!=null)
					{
                   ...
                   <form action="addtocart" method="post">
                   		<input type="hidden" name="uname" value="<%=check%>">
                   		<input type="hidden" name="quantity" value="<%=1%>">
                   		<input type="hidden" name="fname" value="<%=list.get(i).getPfile()%>">
						<input type="hidden" name="pname" value="<%=list.get(i).getPname()%>">
					...
					{
					%>
					<button class="btn btn-outline-warning" onclick="showmsg()">Add to Cart</button>
					<%
					} 
					%>
                  </div>
...
integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDs"></script>
</body>
</html>